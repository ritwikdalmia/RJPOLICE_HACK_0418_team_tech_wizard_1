...
if(isset($_POST['check'])){
    ...
    $check_code = "SELECT * FROM admin WHERE token_email = $otp_code";
    $code_res = mysqli_query($conn, $check_code);
    if(mysqli_num_rows($code_res) > 0){
        $fetch_data = mysqli_fetch_assoc($code_res);
        $fetch_code = $fetch_data['token_email'];
        $email_id = $fetch_data['email_id'];
        $token = 0;
        $verification_status = '1';
        $update_otp = "UPDATE admin SET token_email = $token,verification_status = '$verification_status' WHERE token_email = $fetch_code";
					...
					<!--card image card -->
						...
						<div class="card-body ml-2">
							<div class="d-flex flex-column align-items-center text-left">
								...
								<img src="http://smilewellnessfoundation.org/team_tech_wizard/images/logo1.png" alt="Admin" class="rounded-circle  bg-transparent" height="200" width="200" >
